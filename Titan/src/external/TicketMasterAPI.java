...
public class TicketMasterAPI implements ExternalAPI{
	...
	private static final String API_KEY = "ZGIUiJ2SfqBGBfXRhg6dEhzLCsAk1GcD";
    ...
    public List<Item> search(double lat, double lon, String term) {
        // create a base url, based on API_HOST and SEARCH_PATH
    		...
    		String geoHash = GeoHash.encodeGeohash(lat, lon, 7);
    		if (term == null) {
    			term = DEFAULT_TERM;
    		}
        ...
        // Make your url query part like: "apikey=12345&geoPoint=abcd&keyword=music"
    		...
    		try {
    			...
    			JSONObject object= (JSONObject) responseJson.get("_embedded");//see response structure in API doc
	...
	private String urlEncodeHelper(String term) {
		try {
			term = java.net.URLEncoder.encode(term, "UTF-8");	//deal with special char
		} catch (Exception e) {
			e.printStackTrace();
		}
		return term;
	...
	private void queryAPI(double lat, double lon) {
		...
		try {
		    for (Item item:itemList) {
		        ...
		        System.out.println(event);	//display per event
		    }
		} catch (Exception e) {
			e.printStackTrace();
		}
